...
def get_dataset(dataset, data_path, args):
    if dataset == 'MNIST':
        ...
        im_size = (128, 128)
        num_classes = 2
        mean = [0.7394330916553483, 0.6526700617190773, 0.7774264079340786]
        std = [0.19637706214592562, 0.24239548096074484, 0.1693558713867125]

        # Load annotations and set up transformations
        annotations_path = os.path.join(data_path, 'annotations.csv')
        annotations = pd.read_csv(annotations_path)
        annotations['Majority Vote Label'] = annotations['Majority Vote Label'].apply(lambda x: 1 if x == 'SSA' else 0)
        ...
        # Define function to load an image and its label
        def load_image_and_label(idx):
            img_path = os.path.join(img_dir, annotations.iloc[idx, 0])
            image = Image.open(img_path).convert("RGB")
            label = int(annotations.iloc[idx, 1])
            if transform:
                image = transform(image)
            return image, label

        # Create dataset by loading images and labels
        dataset = [(load_image_and_label(i)) for i in range(len(annotations))]

        # Split dataset into train and test sets
        train_size = 2175
        test_size = len(dataset) - train_size
        dst_train, dst_test = random_split(dataset, [train_size, test_size])

        class_names = ['Normal', 'SSA']
